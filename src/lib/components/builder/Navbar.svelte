<script lang="ts">
	...
	import { ChevronLeft, Smartphone, Tablet, Monitor, Ellipsis, MonitorPlay } from 'lucide-svelte';
...
<nav class="flex h-16 w-full items-center justify-between border-b px-4">
	<div class="flex w-full items-center justify-start gap-x-4 lg:w-1/3">
	...
	<div class="hidden w-1/3 items-center justify-center lg:flex">
		<ToggleGroup.Root type="single" bind:value={$togglePreviewSize}>
			<ToggleGroup.Item
				value="Mobile"
				on:click={togglePreviewSize.setMobile}
			...
			<ToggleGroup.Item
				value="Tablet"
				on:click={togglePreviewSize.setTablet}
			...
			<ToggleGroup.Item
				value="Desktop"
				on:click={togglePreviewSize.setDesktop}
	...
	<div class="hidden w-1/3 items-center justify-end gap-x-2 lg:flex">
		<Tooltip.Root>
			<Tooltip.Trigger asChild let:builder>
				<Button
					...
					builders={[builder]}
					size="icon"
					variant="outline"><MonitorPlay /></Button
				>
			</Tooltip.Trigger>
			<Tooltip.Content>
				<p>Live Preview</p>
